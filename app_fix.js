...
function diagnose() {
  ...
  return { scores, main: ranked[0], sub: ranked[1], plants: pickPlants(ranked[0].id, ranked[1].id, seed) };
}
...
function renderResult(res) {
  ...
  res.plants.forEach((p, idx) => {
    ...
    const imgHtml = src
      ? `<div class="plant-img-wrap"><img class="plant-img" src="${esc(src)}" alt="${esc(p.name)}" loading="lazy"></div>`
      : "";
