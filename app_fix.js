...
/* ===============================
   ...
   - GitHub Pages のパス問題を new URL() で完全吸収
   =============================== */
...
// GitHub Pages の /<repo>/ 配下でも確実に解決する画像URL
function imgUrl(file) {
  return new URL(`assets/plants/${file}`, document.baseURI).toString();
}
...
const TYPES = [
  {
    ...
    name: "余力枯渇タイプ（消耗）",
    summary:
      "回復が追いつかず、日々を“維持”で使い切りがち。まずは世話が少ない成功体験から。",
  },
  {
    id: "overheat",
    name: "思考過熱タイプ（思考オーバーヒート）",
    summary:
      "考えが止まらず、頭が休まらない。視界が整う・リズムが作れる植物が効く。",
  },
  {
    id: "overstim",
    name: "刺激過多タイプ（刺激疲れ）",
    summary:
      "情報・予定・スマホで神経が疲れやすい。低刺激で“静けさ”を増やすのが近道。",
  },
  {
    id: "selfsup",
    name: "自己抑制タイプ（自己後回し）",
    summary:
      "自分を後回しにしがち。『自分のためにやっていい』を増やす相棒が必要。",
  },
];
...
/* -------------------------------
   Scale (A=strongly agree -> E=strongly disagree)
-------------------------------- */
const SCALE = [
...
/* -------------------------------
   Plants (all have summer/winter + image file)
   画像の割り当て（あなたの命名順）
...
/* -------------------------------
   DOM bind (IDs must exist in index.html)
-------------------------------- */
const quizEl = $("#quiz");
...
function diagnose() {
  ...
  answers.forEach((ans, i) => {
    const v = ans ?? 3;
    seed = seed * 31 + v * (i + 1); // changes with answers
...
function pickPlants(mainId, subId, seed) {
  ...
  return picked.slice(0, 3).map((id) => PLANTS[id]).filter(Boolean);
}
